...
SELECT concat(m.firstname, ' ', m.surname) AS name, f.name
FROM `Bookings` AS b
...
SELECT name, user_name,
       (CASE WHEN user_name = 'GUEST GUEST' THEN slots * guestcost
        ...
        AS total_cost
FROM 
(SELECT f.name, concat(m.firstname, ' ', m.surname) as user_name, b.slots, f.guestcost, f.membercost
 FROM `Bookings` AS b
    INNER JOIN `Members` AS m ON b.memid = m.memid
    INNER JOIN `Facilities` AS f ON b.facid = f.facid
...
FROM
(SELECT f.name, concat(m.firstname, ' ', m.surname) as user_name, b.starttime,
       (CASE WHEN m.firstname = 'GUEST' THEN b.slots * f.guestcost
             WHEN m.firstname != 'GUEST' THEN b.slots * f.membercost
        END)
        AS total_cost
...
SELECT m1.surname, m1.firstname, concat(m2.firstname, ' ', m2.surname) AS recommended_by
FROM Members as m1
...
SELECT f.name, concat(m.firstname, ' ', m.surname) as user_name, SUM(slots) as facility_usage 
FROM Bookings as b 
INNER JOIN Facilities as f ON b.facid = f.facid 
INNER JOIN Members as m ON b.memid = m.memid 
